import { ShaderChunk, UniformsLib, Vector2, Color, EventDispatcher, TangentSpaceNormalMap, 
    MultiplyOperation, Material, ShaderMaterial, RawShaderMaterial, ShaderLib, MeshDepthMaterial,
    RGBADepthPacking, Mesh, SkinnedMesh, Color, REVISION } from 'https://unpkg.com/three@0.127.0/build/three.module.js';

// Author: Fyrestar https://mevedia.com (https://github.com/Fyrestar/THREE.extendMaterial)
// Edited by Salome Dieterle
...
function extend( source, object ) {
    ...
    if ( source instanceof Function ) {
        ...
        if ( mapping === undefined ) {
            ...
            console.error( 'extendMaterial: no mapping for material class "%s" found', name );
        ...
        const defaults = ShaderLib[ mapping.name ].uniforms;
    ...
    if ( ( source.isMeshDepthMaterial || source === MeshDepthMaterial ) && defines.DEPTH_PACKING === undefined )
        defines.DEPTH_PACKING = RGBADepthPacking;
...
if ( ShaderMaterial.prototype.customDepthMaterial !== undefined ) {
    ...
    Object.defineProperties( Mesh.prototype, MeshPolyfill );
    Object.defineProperties( SkinnedMesh.prototype, MeshPolyfill );
    ...
    specular: {
        ...
        get: function () {
            ...
            if ( this.uniforms.specular === undefined )
                this.uniforms.specular = { value: new Color( 'white' ) };
...
if ( !Object.isExtensible() || parseInt( REVISION ) > 126 ) {
        ...
        if ( value instanceof Object ) {
            ...
            if ( name[ 0 ] === '@' ) {
                ...
                if ( !chunk ) {
                    ...
                    console.error( "patchShader: chunk not found '%s'", name );
...
function mapUniforms( name, uniforms, object ) {
    ...
    if ( object.explicit === false ) {
        ...
        return cloneUniforms( ShaderLib[ name ].uniforms, uniforms );

    } else {

        // Only use declared and necessary

        return useUniforms( name, object, uniforms ); // cloneUniforms( ShaderLib[ name ].uniforms, uniforms, true );
...
if ( Object.isExtensible() ) {
    ...
    cloneUniforms = this.cloneUniforms;
    cloneUniform = this.cloneUniform;
    patchShader = this.patchShader;
    mapShader = this.mapShader;
    extendMaterial = this.extendMaterial;
